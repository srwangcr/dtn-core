...
src/parser/primary_block.rs: Parser del Bloque Primario BPv7. Expone ParsedBundleHeader<'a>, parse_primary_block() con comprobación estricta de límites e inspección 
O
(
1
)
O(1) vía quick_validate_primary_block().
...
Enrutamiento
src/routing/interval_tree.rs: CgrIntervalTree<const CAP: usize>, un índice estático ordenado por start_time para búsquedas EAT (Earliest Arrival Time). Inserción/eliminación optimizada con copy_within (memmove vectorial) y búsqueda 
O
(
log
⁡
...
Resiliencia contra Fragmentation Overlap: El slot de reensamblado maneja offsets de fragmentos de forma estática en memoria stack/global, garantizando 
O
(
1
)
O(1) en footprint de memoria.
...
📦 Verificación y Release v0.1.0
Ejecutá las verificaciones finales:

fish
cargo test --bin dtnd --bin dtn-cli
cargo check --bin dtn-cli
Subí todo a main y marcá el hito de release con un tag firmado de Git:

fish
git add Dockerfile .dockerignore README.md Cargo.toml src/bin/
git commit -m "feat(release): complete dtn-cli, Dockerfile multi-stage and finalize README for v0.1.0"
git push -u origin feature/fase11-cli-docker-release

# Merge a main
git checkout main
git merge feature/fase11-cli-docker-release
git push origin main

# Tag de Release v0.1.0
git tag -a v0.1.0 -m "Release v0.1.0: Zero-Copy BPv7 Engine with 59.6ns latency"
...
src/parser/primary_block.rs: BPv7 Primary Block parser. Exposes ParsedBundleHeader<'a>, parse_primary_block() with strict bounds checking, and 
O
(
1
)
O(1) inspection via quick_validate_primary_block().
...
Routing
src/routing/interval_tree.rs: CgrIntervalTree<const CAP: usize>, a static index sorted by start_time for EAT (Earliest Arrival Time) lookups. Optimized insertion/deletion with copy_within (vectorized memmove) and 
O
(
log
⁡
...
O(logN) search with find_next(t).
...
Fragmentation Overlap Resilience: The reassembly slot handles fragment offsets statically in stack/global memory, guaranteeing 
O
(
1
)
...
docker build -t dtn-core:v0.1.0 .
docker run -d -p 4556:4556/udp dtn-core:v0.1.0
📦 Verification and Release v0.1.0
Run final verifications:

fish
cargo test --bin dtnd --bin dtn-cli
cargo check --bin dtn-cli
Push everything to main and mark the release milestone with a signed Git tag:
...
git push origin v0.1.0
中文
dtn-core 是一个为高性能裸机和 no_std 环境设计的 DTN 引擎（Bundle Protocol v7，RFC 9171）核心组件实现。其优先考虑：
...
CBOR 和 BPv7 解析器
src/parser/cbor.rs：规范 CBOR 的零拷贝辅助函数。包括用于 VARINT 的 decode_unsigned()、decode_definite_length() 以及针对格式错误 CBOR 的防御。

src/parser/primary_block.rs：BPv7 主块解析器。暴露 ParsedBundleHeader<'a>、带有严格边界检查的 parse_primary_block()，以及通过 quick_validate_primary_block() 实现的 
O
(
1
)
...
src/processor/fragmentation.rs：通过 ReassemblySlot<const BUF_SIZE: usize> 处理分片重组。通过评估 processing_flags 实现分片负载的零分配重建。
...
路由
src/routing/interval_tree.rs：CgrIntervalTree<const CAP: usize>，一个按 start_time 排序的静态索引，用于 EAT（最早到达时间）查找。使用 copy_within（向量化 memmove）优化插入/删除，并通过 find_next(t) 实现 
O
(
log
⁡
...
cargo test --bin dtnd --bin dtn-cli
cargo check --bin dtn-cli
将所有内容推送到 main 并使用签名的 Git 标签标记发布里程碑：
...
src/parser/primary_block.rs: Parser für den BPv7-Primärblock. Stellt ParsedBundleHeader<'a>, parse_primary_block() mit strenger Grenzprüfung und 
O
(
1
)
O(1)-Inspektion über quick_validate_primary_block() bereit.
...
Routing
src/routing/interval_tree.rs: CgrIntervalTree<const CAP: usize>, ein statischer Index sortiert nach start_time für EAT (Earliest Arrival Time)-Suchanfragen. Optimierte Einfügung/Löschung mit copy_within (vektorisiertes memmove) und 
O
(
log
⁡
...
Fragmentierungs-Überlappungs-Resilienz: Der Reassemblierungs-Slot behandelt Fragment-Offsets statisch im Stack/Global-Speicher und garantiert 
O
(
1
)
O(1)-Speicher-Fußabdruck.
...
docker build -t dtn-core:v0.1.0 .
docker run -d -p 4556:4556/udp dtn-core:v0.1.0
📦 Verifikation und Release v0.1.0
Führen Sie die abschließenden Verifikationen durch:

fish
cargo test --bin dtnd --bin dtn-cli
cargo check --bin dtn-cli
Alles auf main pushen und den Release-Meilenstein mit einem signierten Git-Tag markieren:
